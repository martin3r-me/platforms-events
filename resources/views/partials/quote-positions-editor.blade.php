...
<div id="positions-editor-{{ $activeItem->id }}"
     ...
     class="mx-3.5 mb-3.5 bg-white border-2 border-blue-400 rounded-xl overflow-hidden shadow-sm">
    <div class="flex items-center justify-between px-4 py-2.5 bg-blue-50 border-b border-blue-200">
        <div class="flex items-center gap-2.5">
            <div class="w-[3px] h-4 bg-blue-600 rounded-sm"></div>
            <h3 class="text-[0.75rem] font-bold text-[var(--ui-secondary)]">Positionen · {{ $activeItem->typ }}</h3>
            <span class="text-[0.6rem] text-[var(--ui-muted)]">{{ $positions->count() }} Positionen · {{ $fmt($activeItem->umsatz) }} €</span>
        </div>
    ...
    <div class="px-4 py-3 bg-slate-50 border-t border-slate-100 space-y-2">
        ...
        <div class="flex gap-1.5">
            <input wire:model="newPosition.uhrzeit" type="text" placeholder="Uhrzeit (optional)"
            ...
            <input wire:model="newPosition.bemerkung" type="text" placeholder="Bemerkung (optional)"
                   class="flex-[2] border border-slate-200 rounded-md px-2 py-1.5 text-[0.65rem]">
            <button wire:click="addPosition"
                    class="flex items-center gap-1 px-3 py-1.5 rounded bg-blue-600 hover:bg-blue-700 text-white border-0 cursor-pointer text-[0.65rem] font-semibold whitespace-nowrap">
                @svg('heroicon-o-plus', 'w-3 h-3')
                Hinzufügen
            </button>
        </div>
        <p class="text-[0.55rem] text-[var(--ui-muted)]">
            Gesamt-Feld leer lassen ⇒ Anz × Preis wird automatisch berechnet.
        </p>
    </div>
</div>
